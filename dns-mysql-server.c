...
int handle_request(unsigned char *buf,unsigned char *nbuf,int *lock); // Handle web requests
void changetoDnsNameFormat (unsigned char*,unsigned char*);
unsigned char * readName (unsigned char*,unsigned char*,int*);
void dnstoip(unsigned char* qname,unsigned char* phpip);
void stop(int sig);
...
int main(void)
{
    ...
    *lock = 0;
    ...
    while(1)
    {
        ...
        //if(pid ==0)
        //{
	        int size = handle_request(buf,nbuf,lock);
...
int handle_request(unsigned char *buf,unsigned char *nbuf, int *lock)
{
    unsigned char *qname;
    int count=0;
    ...
    int size = strlen(qname)+1;
    if(size<20)size=20;
    unsigned char phpip[size];
    strcpy(phpip,qname);
    //printf("qname: %s\n",qname);
